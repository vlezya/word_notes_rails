class Api::V1::CardDecksController < ApplicationController
  ...
  def index
    ...
    card_decks = CardDeck.joins(:card).where(cards: { user_id: current_user.id }).order(id: :desc)
  ...
  def create
    card_deck = CardDeck.where(card_id: params[:card_id], deck_id: params[:deck_id]).first_or_initialize(card_deck_params)
